	...
	<div class="container">
		<div class="content">
			<div class="form-section">
				...
				<div class="info-box">
					💡 정확한 정보를 입력할수록 반려동물을 찾을 확률이 높아집니다!
				</div>
				...
				<form id="report-form">
					<div class="form-group">
						...
						<input type="text" id="title" name="title" required 
							placeholder="예: 푸들 '뽀삐' 인천 남동구에서 실종됨">
					</div>
					...
					<div class="form-group">
						...
						<textarea id="description" name="description" required
							placeholder="마지막으로 본 위치, 시간, 특이한 특징(흉터, 목걸이 등) 등을 자세히 입력해주세요."></textarea>
					</div>
					...
					<button type="submit" class="btn btn-primary">
						<span id="submit-text">신고 제출</span>
			...
			<div class="map-section">
				<div id="map"></div>
				<div class="map-info">
					지도에서 실종된 위치를 클릭하세요. 마커를 드래그하여 위치를 조정할 수 있습니다.
		...
		document.addEventListener('DOMContentLoaded', function() {
			...
			function addMarkerAtLocation(latlng) {
				...
				marker = L.marker([selectedLat, selectedLng], markerOptions)
					.addTo(map)
					.bindPopup('실종 위치<br>' + selectedLat.toFixed(4) + ', ' + selectedLng.toFixed(4))
					.openPopup();
		...
		document.getElementById('report-form').addEventListener('submit', async function(e) {
			...
			try {
				...
				if (response.ok && data.success) {
					showSuccess('실종 신고가 등록되었습니다! 메인 페이지로 이동합니다.');
					setTimeout(() => {
						window.location.href = 'index.php';
					}, 2000);
				} else {
					showError(data.error || '신고 등록 실패');
				}
			} catch (error) {
