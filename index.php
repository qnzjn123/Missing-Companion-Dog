...
<body>
	...
	<div class="container">
		...
		<div class="sidebar" id="sidebar">
			<div class="tab-buttons">
				...
				<button class="tab-btn" onclick="switchTab('sighting')">??? 목격 제보</button>
			</div>
			...
			<div id="tabReportContent" class="tab-content active">
				...
				<div class="alert-section">
					<h4>?? 알림 설정</h4>
					<div class="filter-row">
						<label style="font-size: 11px;">??? 감시 반경 (km)</label>
	...
	<script>
		...
		function addSightingMarker(sighting) {
			...
			const infowindow = new kakao.maps.InfoWindow({
				content: '<div style="padding:8px;"><h4>??? 목격 제보</h4><p><strong>시간:</strong> ' + sighting.sightingTime + '</p>' + photo + '<p><strong>상태:</strong> ' + sighting.status + '</p>' + (sighting.phone ? '<p><strong>연락:</strong> ' + sighting.phone + '</p>' : '') + '</div>'
			});
		...
		function displaySightingList() {
			...
			filteredSightings.forEach((s, i) => {
				...
				item.innerHTML = '<h4>??? 목격</h4><p><strong>시간:</strong> ' + s.sightingTime + '</p>' + photo + '<p>' + s.status.substring(0, 20) + '...</p>';
				item.onclick = () => {
